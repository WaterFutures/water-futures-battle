...
While we utilize real data from trusted sources where possible (e.g., CBS for municipal data, KNMI for weather observations, Vewin for statistics about water utilities), other aspects rely on assumptions based on expert knowledge.
...
Strategic planning for water distribution networks focuses on the long-term design of the primary supply system – the grid.
...
This part of the system is extremely critical as failures here can propagate and lead to loss of service for thousands of people and businesses.
...
Competitors must embrace uncertainty and decide for themselves what to model and how, reflecting the reality that different teams would make different modelling choices when facing deep uncertainty.
