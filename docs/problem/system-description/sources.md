...
The  $E^{\text{grid}}_s(y)$ represents the energy consumed by the source for the production of water and it is calculate as:
$$
E^{\text{grid}}_s(y) = \sum_{t \in \mathcal{Y}} \max\bigl(E_s(t) - E^{\text{PV}}_n(t), 0\bigr)
$${#eq:energy-consumed}

where $E_p(t)$ represents the energy consumed by the sources $s$ at time  $t$, and the $E^{\text{PV}}_n(t)$ is electricity generated by the photovoltaic panels associated with the source $s$ at time $t$.
